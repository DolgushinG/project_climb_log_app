...
| `subscription_recently_expired` | boolean | **Рекомендуется.** `true` — у пользователя была оплаченная подписка, она истекла (`expires_at < now`), активной подписки нет. Нужно для баннера «Подписка закончилась» при заходе с нового устройства, после переустановки и т.п., когда клиентский кэш отсутствует. |

**Логика `subscription_recently_expired`:**
- `true` — есть последняя запись в `premium_subscriptions` с `expires_at < now()` (подписка истекла), и нет активной подписки. Иначе `false`.
- Без этого поля баннер показывается только при совпадении клиентского кэша (ненадёжно при новом устройстве, переустановке).
...
| Подписка истекла | Баннер «Подписка закончилась. Оформите снова» (см. [PREMIUM_EXPIRED_BANNER_SPEC.md](./PREMIUM_EXPIRED_BANNER_SPEC.md)) |
