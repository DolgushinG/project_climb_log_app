  ...
  "completions": [
    {
      ...
      "exercise_name_ru": "Репитеры 7:13",
      "sets_done": 3,
      "weight_kg": 25.5
    }
  ]
...
Рекомендуется возвращать `exercise_name_ru` — в приложении для отображения всегда приоритетно русское название.

**Упражнения тренера:** `exercise_id` может быть в формате `trainer_{id}_{n}` (собственные упражнения тренера). POST и GET должны принимать и возвращать такие ID без фильтрации — иначе выполнение заданий от тренера не отобразится.
