...
| 403 | AI-провайдер не настроен, отключён, или блокировка за нарушения. При блокировке за нарушения — тело ответа содержит `code: "ai_banned"` и `error` с текстом сообщения |
| 404 | `conversation_id` передан, но диалог не найден или принадлежит другому пользователю |
| 503 | Ошибка AI (таймаут, лимиты и т.п.) |

### Обработка 403 с `ai_banned`

При бане за нарушение правил API возвращает **403** с телом:
```json
{
  "code": "ai_banned",
  "error": "Доступ заблокирован из-за нарушения правил использования. Пожалуйста, соблюдайте правила сервиса."
}
```

Клиент должен проверять `code == "ai_banned"` и показывать сообщение из `error` (или текст по умолчанию). Другие 403 (нет премиума, провайдер, возраст) обрабатываются отдельно.
